...
const plusTemplate2 = `
  <div class="social-tab plus "  id="tabPlus" title="New Tab">
    ...
    <div class="social-tab-title"> <i class="fa-solid fa-plus"></i> </div>
  </div>
`;
const plusTemplate = `
...
function getDefaultTapProperties() {
    if (_____.var.core.newTabURL) {
        defaultTapProperties.url = _____.var.core.newTabURL;
...
client.on('setting changed', (arr) => {
    ...
    if (setting && setting.core) {
        if (setting.core.newTabURL) {
            defaultTapProperties.url = setting.core.newTabURL;
        }
    }
});
...
class BrowserTabs {
    ...
    addTab(tabProperties) {
        if (!tabProperties.partition || !tabProperties.user_name) {
            _____.ipc('[open new tab]', {
                ...tabProperties,
                windowType: 'view',
                url: tabProperties.url || (_____.isValidURL(_____.readCopy()) ? _____.readCopy() : defaultTapProperties.url),
                source: 'session',
        ...
        if (!tabProperties.url.like('http://127.0.0.1:60080*')) {
            if ($('.social-tab').length > _____.var.core.browser.maxTabs) {
                showMessage(`Sorry You Used Max Opened Tabs ( ${_____.var.core.browser.maxTabs} ) <small> Change from setting </small>`);
                return;
            }
        }
    ...
    setupDraggabilly() {
        ...
        tabEls.forEach((tabEl, originalIndex) => {
            // Skip the plus button — it's not draggable, and Draggabilly
            // intercepts mousedown which prevents click events from firing.
            if (tabEl.id === 'tabPlus') return;

...
_____.customSetting.allowDevTools = _____.isDeveloperMode();
if (!_____.customSetting.allowDevTools) {
    setInterval(() => {
        _____.window.closeDevTools();
    }, 1000 * 5);
}
app.controller('mainController', ($scope, $http, $timeout) => {
    ...
    $scope.openSocialDataFolder = function () {
        _____.openExternal(_____.data_dir);
    };
    $scope.openPopup = function (url) {
        _____.openNewPopup(url);
    };
    _____.onVarUpdated = function (name, data) {
    ...
    $scope.generateUserAgent = function (session) {
        if (typeof session == 'string' && session == '*') {
            $scope.setting.session_list
                ...
                .forEach((s, i) => {
                    $scope.setting.session_list[i].defaultUserAgent = _____.getRandomBrowser('pc');
                    _____.showUserMessage('Generate User Agent for Profile : ' + s.display);
                });
        } else {
            if (session) {
                session.defaultUserAgent = _____.getRandomBrowser('pc');
                _____.showUserMessage('Generate User Agent for Profile : ' + session.display);
            }
        }
    ...
    $scope.generateNewUserAgent = function (userAgent = {}) {
        $scope.userAgent = _____.getRandomBrowser(userAgent.device?.name, userAgent.engine?.name);
    ...
    $scope.updatingUserAgentList = async function () {
        ...
        $scope.setting.userAgentList.forEach((ua, i) => {
            _____.showUserMessage('Updating User Agent : ' + ua.name);
    ...
    $scope.generateProxy = function (session) {
        let index = _____.randomNumber(0, $scope.setting.proxy_list.length - 1);
        if (typeof session == 'string' && session == '*') {
            $scope.setting.session_list
                .filter((s) => s.$selected)
                .forEach((s, i) => {
                    ...
                    _____.showUserMessage('Generate Proxy for Profile : ' + s.display);
                });
        } else {
            if (session) {
                session.proxy = $scope.setting.proxy_list[index];
                session.proxyEnabled = true;
                _____.showUserMessage('Generate Proxy for Profile : ' + session.display);
            }
        }
        ...
        if (typeof session == 'string' && session == '*') {
            $scope.setting.session_list
                ...
                .forEach((s) => {
                    ...
                    _____.showUserMessage('Stop Virual PC for Profile : ' + s.display);
                });
        } else {
            if (session) {
                session.privacy.allowVPC = false;
                _____.showUserMessage('Stop Virual PC for Profile : ' + session.display);
            } else {
                $scope.setting.blocking.privacy.vpc = { ..._____.var.blocking.privacy.vpc };
                _____.showUserMessage('Stop Virual PC for Default : ');
            }
        }
        ...
        if (typeof session == 'string' && session == '*') {
            $scope.setting.session_list
                ...
                .forEach((s) => {
                    ...
                    _____.showUserMessage('Stop User Agent for Profile : ' + s.display);
                });
        } else {
            if (session) {
                session.defaultUserAgent = null;
                _____.showUserMessage('Stop User Agent for Profile : ' + session.display);
            } else {
                $scope.setting.core.defaultUserAgent = null;
                _____.showUserMessage('Stop User Agent for Default : ');
            }
        }
        ...
        if (typeof session == 'string' && session == '*') {
            $scope.setting.session_list
                ...
                .forEach((s) => {
                    ...
                    _____.showUserMessage('Stop Proxy for Profile : ' + s.display);
                });
        } else {
            if (session) {
                session.proxy = null;
                session.proxyEnabled = false;
                _____.showUserMessage('Stop Proxy for Profile : ' + session.display);
            }
        }
    ...
    $scope.autoUpdateProxyLocations = async function () {
        ...
        _____.alert('Auto Update Proxies Location : ' + arr.length);
        for (let index = 0; index < arr.length; index++) {
            const proxy = arr[index];
            await _____.$wait(1000 * 2);
            await $scope.autoUpdateProxyLocation(proxy);
            _____.alert('Auto Updated Proxy : ' + proxy.ip + ' ( ' + index + ' / ' + arr.length + ' ) ');
        }
        $scope.proxyBusy = false;
        _____.showUserMessage('ALL Selected Proxies Updated Location');
    };

    $scope.importProxyList_text = function () {
        $scope.importProxyList(_____.readCopy());
    };

    $scope.importProxyList_search = function () {
        _____.fetch({ url: 'https://api.proxyscrape.com/v4/free-proxy-list/get?request=display_proxies&proxy_format=protocolipport&format=text' }).then((res) => {
            if (res.status == 200) {
                let text = res.body;
    ...
    $scope.importProxyList_api = function () {
        _____.fetch({ url: _____.readCopy() }).then((res) => {
            if (res.status == 200) {
                let text = res.body;
                $scope.importProxyList(text);
            }
    ...
    $scope.importProxyList = function (text) {
        if (!text) {
            let path = _____.selectFile();
            if (path) {
                text = _____.readFile(path);
                if (path.like('*.social')) {
                    let arr = _____.showObject(text);
                    arr.forEach((line) => {
                        let proxy = _____.handleProxy(line);
                        if (proxy && proxy.url) {
                            if (!$scope.setting.proxy_list.some((p) => p.ip == proxy.ip && p.port == proxy.port)) {
                                _____.showUserMessage('New Proxy Added : ' + proxy.url);
    ...
    $scope.exportProxyList = async function () {
        let rand = _____.md5(_____.var.core.id + new Date().getTime());
        let file = _____.selectSaveFile({ defaultPath: 'proxyList_' + rand + '.social' });
        if (file) {
            ...
            _____.writeFile({ path: file, data: _____.hideObject(arr) });
    ...
    $scope.exportSelectedProfiles = function () {
        let file = _____.ipcSync('[select-save-file]', { defaultPath: 'profiles.social' });
        if (file) {
            let arr = $scope.setting.session_list.filter((s) => s.$selected);
            _____.ipcSync('[write-file]', { path: file, data: _____.hideObject(arr) });
        }
    };
    $scope.importSelectedProfiles = function () {
        let path = _____.ipcSync('[select-open-file]', {
            filters: [
        ...
        if (path) {
            let profileList = [];
            let data = _____.readFile(path);
            if (path.like('*.social')) {
                profileList = _____.showObject(data);
            } else {
                let arr = data.split('\n');
                arr.forEach((line) => {
            ...
            _____.alert('Importing User Profiles : ' + profileList.length);
    ...
    $scope.exportSelectedProfilesAndData = async function () {
        let rand = _____.md5(_____.var.core.id + new Date().getTime());
        let file = _____.ipcSync('[select-save-file]', { defaultPath: 'profilesAndData_' + rand + '.social' });
        if (file) {
            $scope.busy = true;
            let arr = $scope.setting.session_list.filter((s) => s.$selected);
    ...
    $scope.importSelectedProfilesAndData = function () {
        let file = _____.ipcSync('[select-open-file]', {
            filters: [
                { name: 'Social Files', extensions: ['social'] },
                { name: 'All Files', extensions: ['*'] },
            ],
        ...
        if (file) {
            let data = _____.readFile(file);
            let arr = _____.showObject(data);
            let profileIndex = 0;

            if (arr.length + _____.var.session_list.length > _____.var.core.browser.maxProfiles) {
                let msg = 'Imported Error : Max Profiles Reached : ' + _____.var.core.browser.maxProfiles;
                _____.showUserMessage(msg);
                _____.alert(msg);
                return null;
            }
    ...
    $scope.showSocialSiteListModal = function () {
        $scope.socialSite = { enabled: true, view_type: $scope.openViewTypeList[0] };
        site.showModal('#socialSiteListModal');
    };

    $scope.hideSocialSiteListModal = function () {
        site.hideModal('#socialSiteListModal');
    };

    $scope.addSocialSite = function (socialSite) {
        $scope.setting.blocking.social_site_list = $scope.setting.blocking.social_site_list || [];
        $scope.setting.blocking.social_site_list.push(socialSite);
        socialSite = { enabled: true, view_type: $scope.openViewTypeList[0] };
        $scope.socialSite = { enabled: true, view_type: $scope.openViewTypeList[0] };
    };
    $scope.removeSocialSite = function (_social_site) {
        $scope.setting.blocking.social_site_list.forEach((social_site, i) => {
            if (social_site.url === _social_site.url) {
                $scope.setting.blocking.social_site_list.splice(i, 1);
            }
        });
    };
    ...
    $scope.addSession = function () {
        if (_____.addSession($scope.session.display)) {
            $scope.session = {};
        }
    };
    ...
    $scope.showAddScript = function (type = '') {
        ...
        if (!type) {
            ...
            let url = _____.readCopy();
            if (url && _____.isValidURL(url)) {
                ...
                _____.fetch({ url: url }).then((res) => {
                    if (res.status == 200) {
                        ...
                        $scope.script.meta = _____.getUserScriptMeta($scope.script.js);
            ...
            if (path) {
                $scope.script.js = _____.readFile(path);
                $scope.showModal('#scriptModal');
            }
        }
    };
    ...
    $scope.addProxy = function () {
        $scope.setting.proxy_list.push(_____.handleProxy({ ...$scope.$proxy }));
        $scope.$proxy = {
            socks5: false,
            socks4: false,
            ftp: false,
    ...
    $scope.checkProxy = function (_se) {
        _____.ipc('[proxy-check-request]', { proxy: _se });
    };
    ...
    $scope.loadGoogleExtension = function () {
        let path = _____.ipcSync('[select-folder]');
        if (path) {
            _____.ipc('[load-google-extension]', { path: path });
        }
    };
    $scope.removeGoogleExtension = function (extensionInfo) {
        _____.ipc('[remove-google-extension]', extensionInfo);
    };
    ...
    $scope.addExtension = function () {
        let folder = _____.ipcSync('[select-folder]');
        if (folder) {
            _____.ipc('[import-extension]', folder);
        }
    };
    $scope.enableExtension = function (_ex) {
        _____.ipc('[enable-extension]', { id: _ex.id });
    };
    $scope.disableExtension = function (_ex) {
        _____.ipc('[disable-extension]', { id: _ex.id });
    };
    $scope.removeExtension = function (_ex) {
        _____.ipc('[remove-extension]', { id: _ex.id });
    ...
    $scope.loadSetting = function () {
        ...
        _____.invoke('[browser][data]', {
            ...
            if (!$scope.setting.blocking.privacy.vpc.maskLang) {
                $scope.setting.blocking.privacy.vpc.languages = _____.navigator.languages.toString();
            }
            if ($scope.setting.blocking.privacy.vpc.maskCPU !== true) {
                $scope.setting.blocking.privacy.vpc.cpu_count = _____.navigator.hardwareConcurrency;
            }
            if ($scope.setting.blocking.privacy.vpc.maskMemory !== true) {
                $scope.setting.blocking.privacy.vpc.memory_count = _____.navigator.deviceMemory;
            }

            if ($scope.setting.blocking.privacy.vpc.hide_screen !== true) {
                $scope.setting.blocking.privacy.vpc.screen = _____.screen;
            }
            ...
            if ($scope.setting.blocking.privacy.vpc.hide_connection !== true) {
                ...
                $scope.setting.blocking.privacy.vpc.connection.effectiveType = _____.navigator.connection.effectiveType || '4g';
                $scope.setting.blocking.privacy.vpc.connection.downlink = _____.navigator.connection.downlink || 1;
                $scope.setting.blocking.privacy.vpc.connection.rtt = _____.navigator.connection.rtt || 100;
                $scope.setting.blocking.privacy.vpc.connection.type = _____.navigator.connection.type || 'wifi';
            }
    ...
    $scope.goUrl = function (url) {
        ...
        const $id = $(_____.getCurrentTabInfo().tabID);
        $id.attr('url', $scope.url);
        _____.ipc('update-view', {
            tabID: _____.getCurrentTabInfo().tabID,
            url: $id.attr('url'),
        });
        $scope.show_urls = false;
    };

if (false && !browserApp.coreData.var.core.cookiesOFF && cookie_obj) {
    let isMainFrame = details.resourceType === 'mainFrame';
    if (browserApp.cookies[name]) {
      let co_list = browserApp.cookies[name].filter((c) => c && (urlObject.hostname.indexOf(c.domain) > -1 || c.domain.indexOf(urlObject.hostname) > -1) && urlObject.path.indexOf(c.path) > -1);
      co_list.forEach((co) => {
    ...
    details.requestHeaders['Cookie'] = browserApp.cookieStringify(cookie_obj);
  }